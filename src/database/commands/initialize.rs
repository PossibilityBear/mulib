...
use crate::{
    database::{commands::songs::get_all_songs, db_models::*},
    models::{album::ParsedAlbum, artist::ParsedArtist, song::ParsedSong},
};
use sqlx::Error;
...
pub async fn initialize_db(conn: &DbConnection) -> Result<(), Error> {
    let music_dir = &env::var(config_keys::MUSIC_DIR).expect(&format!(
        "Expected environment var for '{}' to be set",
        config_keys::MUSIC_DIR
    ));
    let local_songs = read_music(music_dir.into())
        .expect(&format!("Expected music dir: '{}' to exist", music_dir));
    println!("Read {} Songs From Local Library", local_songs.len());

    let db_songs: Vec<ParsedSong> = get_all_songs(conn)
        .await?
        .into_iter()
        .map(|s| s.into())
        .collect();
...
fn diff_songs(
    local: Vec<ParsedSong>,
    db: Vec<ParsedSong>,
) -> (Vec<ParsedSong>, Vec<ParsedSong>, Vec<ParsedSong>) {
    let new: Vec<ParsedSong> = local
        .iter()
        .filter(|l| None == db.iter().find(|d| d.file_path == l.file_path))
        .map(|l| l.clone())
        .collect();

    let updated: Vec<ParsedSong> = local
        .iter()
    ...
    let removed: Vec<ParsedSong> = db
        .into_iter()
        .filter(|d| None == local.iter().find(|l| l.file_path == d.file_path))
        .collect();
...
async fn init_albums(
...
) -> Result<HashMap<String, i64>, Error> {
    {
        ...
        let mut tx = conn.db.begin().await?;
        for album in &albums {
            let artist_id = if let Some(artist) = &album.artist {
...
async fn init_songs(
    ...
    for song in songs {
        let artist_id: Option<i64> = match &song.artist {
            Some(artist) => Some(*artists.get(&artist.name).unwrap()),
            None => None,
        };

        let album_id: Option<i64> = match &song.album {
            Some(album) => Some(*albums.get(&album.title).unwrap()),
            None => None,
        };
